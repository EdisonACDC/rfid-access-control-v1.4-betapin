...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        async def _execute_user_actions(user, keypad_action: str = ""):
            """Execute matching actions for a validated user."""
            user.record_access()
            ...
            for action in user.actions:
                if keypad_action and not action.matches_keypad_action(keypad_action):
                    _LOGGER.debug(
                        f"Skipping action '{action.action_name}' "
                        f"(trigger={action.keypad_action}, keypad={keypad_action})"
                    )
                    continue
                try:
                    if action.delay_before_seconds > 0:
                        _LOGGER.info(
                            f"Attendo {action.delay_before_seconds}s PRIMA di '{action.action_name}'"
                        )
                        await asyncio.sleep(action.delay_before_seconds)
                    ...
                    if len(domain_svc) == 2:
                        ...
                        executed += 1
                        _LOGGER.info(
                            f"Azione '{action.action_name}' eseguita per "
                            f"{user.user_name} (keypad={keypad_action}, "
                            f"delay_before={action.delay_before_seconds}s, "
                            f"delay_after={action.delay_after_seconds}s)"
                        )
                    else:
                        _LOGGER.error(f"Formato servizio non valido: {action.service}")

                    if action.delay_after_seconds > 0:
                        _LOGGER.info(
                            f"Attendo {action.delay_after_seconds}s DOPO '{action.action_name}'"
                        )
                        await asyncio.sleep(action.delay_after_seconds)

                except Exception as e:
                    _LOGGER.error(f"Errore esecuzione azione '{action.action_name}': {e}")

            _LOGGER.info(
                f"Access GRANTED: {user.user_name} | keypad={keypad_action} | "
                f"actions executed={executed}/{len(user.actions)}"
            )
        ...
        async def _mqtt_validate(action_code, action_type):
            ...
            if user:
                await _execute_user_actions(user, keypad_action=action_type)
            else:
                hass.bus.async_fire(EVENT_ACCESS_DENIED, {
                    "code_type": "rfid" if is_rfid else "pin",
                    "code_hint": action_code[-4:] if is_rfid else "***",
                    "keypad_action": action_type,
                })
        ...
        async def handle_add_action(call: ServiceCall) -> None:
            ...
            try:
                ...
                action = AccessAction(
                    ...
                    keypad_action=call.data.get("keypad_action", "any"),
                    delay_before_seconds=int(call.data.get("delay_before_seconds", 0)),
                    delay_after_seconds=int(call.data.get("delay_after_seconds", 0)),
                )
        ...
        async def handle_set_admin_pin(call: ServiceCall) -> None:
            """Change the admin PIN and persist it in storage."""
            try:
                new_pin = str(call.data.get("new_pin", "")).strip()
                if len(new_pin) < 4:
                    _LOGGER.error("Admin PIN deve essere di almeno 4 cifre")
                    return
                db.admin_pin = new_pin
                await _save_database(hass, store_path, db)
                _update_sensor(hass, entry)
                _LOGGER.info("Admin PIN aggiornato e salvato nello storage")
            except Exception as e:
                _LOGGER.error(f"Errore cambio admin PIN: {e}")
        ...
        for service_name, handler in [
            ...
            (SERVICE_SET_ADMIN_PIN, handle_set_admin_pin),
        ]:
